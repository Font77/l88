...
{ Initialise(); Create(pParent, uCallbackMessage, szToolTip, icon, uID); } 
...
BOOL CSystemTray::Create(CWnd* pParent, UINT uCallbackMessage, LPCTSTR szToolTip, HICON icon, UINT uID)
{
    ...
    wcscpy_s(m_tnd.szTip, szToolTip);//_tcscpy(m_tnd.szTip, szToolTip);
...
{
    if (m_bEnabled && !m_bHidden) { m_tnd.uFlags = NIF_ICON; Shell_NotifyIcon (NIM_DELETE, &m_tnd); m_bHidden = TRUE; }
}

void CSystemTray::ShowIcon()
{
    if (m_bEnabled && m_bHidden)
		{ m_tnd.uFlags = NIF_MESSAGE | NIF_ICON | NIF_TIP; Shell_NotifyIcon(NIM_ADD, &m_tnd); m_bHidden = FALSE; }
}
...
{
    m_IconList.RemoveAll();
    try { for (UINT i = 0; i <= nNumIcons; i++) m_IconList.Add(pHIconList[i]); }
    catch (CMemoryException *e) { e->ReportError(); e->Delete(); m_IconList.RemoveAll(); return FALSE; }
    return TRUE;
}
...
{
    if (!m_IconList.GetSize()) return FALSE;
    m_nCurrentIcon++;
    if (m_nCurrentIcon >= m_IconList.GetSize()) m_nCurrentIcon = 0;
...
{
    BOOL bResult = FALSE;
    if (m_uIDTimer) bResult = KillTimer(m_uIDTimer);
    m_uIDTimer = 0;
...
BOOL CSystemTray::SetTooltipText(LPCTSTR pszTip)
{
    ...
    wcscpy_s(m_tnd.szTip, pszTip);//_tcscpy(m_tnd.szTip, pszTip);
    return Shell_NotifyIcon(NIM_MODIFY, &m_tnd);
}
...
{
    CString strText;
    if (m_bEnabled) strText = m_tnd.szTip;
    return strText;
}
...
BOOL CSystemTray::SetMenuDefaultItem(UINT uItem, BOOL bByPos)
{
    ...
    m_DefaultMenuItemID = uItem; m_DefaultMenuItemByPos = bByPos;
    CMenu menu, *pSubMenu;
...
void CSystemTray::OnTimer(UINT nIDEvent) 
{
    ...
    if (m_nAnimationPeriod > 0 && m_nAnimationPeriod < period.GetTotalSeconds()) { StopAnimation(); return; }
    StepAnimation();
}
...
{
	if (message == m_tnd.uCallbackMessage) return OnTrayNotification(wParam, lParam);	
