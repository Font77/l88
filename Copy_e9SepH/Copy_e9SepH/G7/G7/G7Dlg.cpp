...
// CG7Dlg dialog
CG7Dlg::CG7Dlg(CWnd* pParent /*=NULL*/)
	: CDialog(CG7Dlg::IDD, pParent)
{
   ...
   lcd = new LCD16(4,236,2,2);
}

void CG7Dlg::DoDataExchange(CDataExchange* pDX)
{
   CDialog::DoDataExchange(pDX);
}

BEGIN_MESSAGE_MAP(CG7Dlg, CDialog)
   ON_WM_PAINT()
   ON_WM_QUERYDRAGICON()
   //}}AFX_MSG_MAP
   ON_CBN_SELCHANGE(IDC_COMBO1, &CG7Dlg::OnCbnSelchangeCombo1)
   ON_BN_CLICKED(IDC_CHECK1, &CG7Dlg::OnBnClickedCheck1)
   ON_BN_CLICKED(IDC_CHECK2, &CG7Dlg::OnBnClickedCheck2)
   ON_BN_CLICKED(IDC_CHECK3, &CG7Dlg::OnBnClickedCheck3)
   ON_WM_NCLBUTTONDOWN()
   ON_WM_MOUSEMOVE()
   ON_WM_LBUTTONDOWN()
   ON_WM_LBUTTONDBLCLK()
   ON_BN_CLICKED(IDC_CHECK4, &CG7Dlg::OnBnClickedCheck4)
...
// CG7Dlg message handlers
BOOL CG7Dlg::OnInitDialog()
{
   ...
   p_cbox->SetCurSel(0);
   VERIFY(cSystray.Create(NULL, WM_ICON_NOTIFY, _T("G7"), m_hIcon, IDR_MENU1));
...
HCURSOR CG7Dlg::OnQueryDragIcon()
{
   return static_cast<HCURSOR>(m_hIcon);
}
void CG7Dlg::DrawKeyboard(void)
{
   CClientDC dc(this);
   CBitmap bitmap; 
   CDC dcMemory;
   // Sequence is important
   dc.MoveTo(0,32); dc.LineTo(264,32);dc.MoveTo(0,0);
   bitmap.LoadBitmap(IDB_BITMAP1);
   dcMemory.CreateCompatibleDC(&dc);
   dcMemory.SelectObject(&bitmap);
   dc.BitBlt(2, 34, 254, 133, &dcMemory, 0, 0, SRCCOPY);
}
void CG7Dlg::Draw2LEDs()
{
   CClientDC dc(this);
   CBrush brushBlack(RGB(0, 0, 0)); CBrush brushRed(RGB(255, 0, 0));
   for(int i=0;i<8;i++) { dc.FillRect( &(lcd->rectLED[i]), &brushBlack ); }
   dc.FillRect( &(lcd->rectLED[16]), &brushBlack );
   for(int i=8;i<16;i++) { dc.FillRect( &(lcd->rectLED[i]), &brushRed ); }
   dc.FillRect( &(lcd->rectLED[17]), &brushRed );
   brushBlack.DeleteObject(); brushRed.DeleteObject();
}
int CG7Dlg::DescribeKeyState() 
{
...
void CG7Dlg::OnNcLButtonDown(UINT nHitTest, CPoint point)
{
...
void CG7Dlg::OnMouseMove(UINT nFlags, CPoint point)
{
// TODO: Add your message handler code here and/or call default
   SetActiveWindow();
...
void CG7Dlg::processKeyDaunCommon(int buttonIndex)
{
   int _bitCheckerSetter = 1<<buttonIndex;
   _16bitsMkcVal ^= _bitCheckerSetter ;
   //_state16Keys |= _bitCheckerSetter ;
   //_state16Keys ^= _bitCheckerSetter ;
   lcd->fill2LEDs(_16bitsMkcVal);
...
void CG7Dlg::syncCheckBoxes(void)
{
   //int state = 0;
   short ks;
...
void CG7Dlg::OnKeyUp(UINT nChar, UINT nRepCnt, UINT nFlags)
{
   syncCheckBoxes();
   //CDialogEx::OnKeyDown(nChar, nRepCnt, nFlags);
}
void CG7Dlg::OnLButtonDblClk(UINT nFlags, CPoint point)
{
   // TODO: Add your message handler code here and/or call default
...
void CG7Dlg::OnLButtonDown(UINT nFlags, CPoint point)
{
   // TODO: Add your message handler code here and/or call default
...
void CG7Dlg::OnCbnSelchangeCombo1()
{
...
void CG7Dlg::OnShowWindow(BOOL bShow, UINT nStatus)
{
...
void CG7Dlg::OnShowKeyboard() {	ShowWindow(SW_SHOW); }
void CG7Dlg::OnUpdateShowKeyboard(CCmdUI* pCmdUI) {	pCmdUI->Enable(!IsWindowVisible()); }
void CG7Dlg::OnHideKeyboard()
{
	int state = DescribeKeyState();
	if(state & 0x01) keybd_event(VK_CAPITAL,0,KEYEVENTF_KEYUP,0);
	if(state & 0x02) keybd_event(VK_SHIFT,0,KEYEVENTF_KEYUP,0);
...
void CG7Dlg::OnUpdateHideKeyboard(CCmdUI* pCmdUI) {	pCmdUI->Enable(IsWindowVisible()); }
void CG7Dlg::OnExit() 
{
	int state = DescribeKeyState();
	if(state & 0x01) keybd_event(VK_CAPITAL,0,KEYEVENTF_KEYUP,0);
